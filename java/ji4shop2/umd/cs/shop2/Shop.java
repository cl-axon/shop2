 ...
 * @see #Shop(String, String) Shop standalone constructor
 */
...
/**
 * The standalone constructor starts SHOP2 (unless it is already
 * running), and then loads the specified domain file and problem
 * file, and then it has SHOP2 solve all of the problems that are
 * defined in the problem file (printing the results as it goes).  The
 * {@link #tree tree}, {@link #addList addList}, and {@link
 * #deleteList deleteList} fields are set by this constructor to be
 * the plan tree, add list, and delete list for the <i>last</i>
 * problem solved.  <p>This is the contructor invoked by the {@link
 * #main(String []) main} routine.  It is primarily created for use in
 * stand-alone operation of SHOP2, and it does not provide much
 * fine-grained control (i.e., it runs all of the plans in the given
 * problem file and it prints out all of the results that it
 * produces).
 *
 ...
 * @see #Shop(String, State, TaskAtom) simple embedded constructor 
 * @see #Shop(String, State, TaskAtom, String, String, QueryWatcher)
 * complex embedded constructor 
 */
    ...
    /**
     * The complex embedded constructor starts SHOP2 (unless it is
     * already running), and then loads the specified domain file, and
     * then it has SHOP2 create a plan for the specified task in the
     * specified state.  The {@link #tree tree}, {@link #addList
     * addList}, and {@link #deleteList deleteList} fields are set by
     * this constructor to be the plan tree, add list, and delete list
     ...
     * @param extraFile An extra Lisp file containing code specific to the
     * @param extraArgs Extra arguments passed to SHOP2.
     * @see #Shop(String, String) standalone constructor
     * @see #Shop(String, State, TaskAtom) simple embedded constructor
     */
    public Shop( String nameDomainFile, State startState, TaskAtom task,
		 String extraFile, String extraArgs, QueryWatcher queryW )
    {
	...
	if (!extraFile.equals(""))
	    retval = jl.requestLisp("(load \"" + extraFile + "\")");

	solveProblem("jshop-problem", extraArgs, queryW);
	retval = jl.requestLisp("(jshop-next-plan)");
	extractResults(taskListString);
     }

    /**
     * The simple embedded constructor starts SHOP2 (unless it is
     * already running), and then loads the specified domain file, and
     * then it has SHOP2 create a plan for the specified task in the
     * specified state.  The {@link #tree tree}, {@link #addList
     * addList}, and {@link #deleteList deleteList} fields are set by
     * this constructor to be the plan tree, add list, and delete list
     * for the task and state.
     * <p>This is a simplified version of the #Shop(String, State,
     * TaskAtom, String, String) constructor; it does not load any
     * extra Lisp file or extra argument.
     *
     * @param nameDomainFile The path and file name for a file that contains a
     * SHOP2 domain description (i.e., a defdomain or make-domain statement).
     * @param startState The world state at the start of the planning process.
     ...
     * or make-problem statements).
     * @param task The main task for which a plan should be created.
     * @see #Shop(String, String) standalone contructor
     * @see #Shop(String, State, TaskAtom, String, String, QueryWatcher) 
     * complex embedded constructor
     */
    public Shop( String nameDomainFile, State startState, TaskAtom task)
    {
	this(nameDomainFile, startState, task, "", "", null);
    }

    /**
     * Sets the {@link #verbosity}.
     *
     * @param v The new verbosity (0-3).
     */
	...
	if (jl == null)
		...
		jl.requestLisp("(in-package :user)");
	    }
    }
     ...
     * @param extraArgs Extra arguments (if any) for SHOP2.
     * @param queryW A query watcher for accessing external data.
     */
    private void solveProblem ( String problemName, String extraArgs,
				QueryWatcher queryW)
    {
	String whichProbs, retval;

	retval = jl.requestLisp("(jshop-find-plans '" + problemName 
				+ " :verbose " + verbosity
				+ " " + extraArgs + ")");

	while ((retval.length() > 0) & (retval.charAt(0)==')'))
	    retval = jl.requestLisp(queryW.processQuery(retval.substring(1)));
    }
