...
%type <t_timed_effect> c_timed_effect
%type <t_quantifier> c_quantifier
...
%type <t_proposition> c_proposition c_init_proposition
...
%type <t_type_list> c_primitive_types c_new_primitive_types c_typed_types c_type_names c_primitive_types c_either_type

%type <t_goal> c_goal_descriptor c_goal_spec c_duration_constraint c_da_gd c_timed_gd /* c_f_comp */
...
%type <t_expression> c_f_exp c_ground_f_exp c_binary_ground_f_exp c_f_exp_da c_f_exp_t c_binary_expr_da c_d_value
%type <t_num_expression> c_number
%type <t_assign_op> /* c_assign_op */ /* c_assign_op_t */
%type <t_comparison_op> c_comparison_op c_d_op

%type <t_operator> c_structure_def
...
%type <t_operator_list> c_structure_defs
...
%type <t_dummy> /*c_action_kind*/ c_args_head
...
%token <punct> OPEN_BRAC CLOSE_BRAC 
       OPEN_SQ CLOSE_SQ
       DEFINE DOMAIN REQS EQUALITY STRIPS ADL NEGATIVE_PRECONDITIONS
       ...
       ACTION PROCESS EVENT DURATIVE_ACTION
       CONSTANTS PREDS FUNCTIONS TYPES ARGS PRE CONDITION
...
c_domain_name : OPEN_BRAC DOMAIN NAME CLOSE_BRAC {$$=$3;}
...
c_new_func_symbol :
     NAME 
         { $$=current_analysis->func_tab.symbol_put($1);
           current_analysis->var_tab_stack.push(new var_symbol_table); delete [] $1; }
;
    ...
    CLOSE_BRAC
        { $$= new effect_lists; 
          $$->forall_effects.push_back(
	       new forall_effect($6, current_analysis->var_tab_stack.pop())); 
          requires(E_COND_EFFS); 
          $4->clear();delete $4;}
...
|   c_structure_def  { $$= new operator_list; $$->push_back($1); }
;
...
c_action_def :
    ...
    EFFECTS c_effect
    CLOSE_BRAC
    { $$= new action(current_analysis->op_tab.symbol_put($3),
			$6,$9,$11,
...
c_event_def :
    ...
    EFFECTS c_effect /* $11 */
    CLOSE_BRAC
    {$$= new event(current_analysis->op_tab.symbol_put($3),
		   $6,$9,$11,
		   current_analysis->var_tab_stack.pop()); delete [] $3;}
...
c_process_def :
    ...
    EFFECTS c_effect
    CLOSE_BRAC
    {$$= new process(current_analysis->op_tab.symbol_put($3),
		     $6,$9,$11,
                     current_analysis->var_tab_stack.pop()); delete [] $3;}
...
c_args_head :
    ARGS {$$= 0; current_analysis->var_tab_stack.push(new var_symbol_table);}
;
...
c_goal_spec : OPEN_BRAC GOALS c_goal_descriptor CLOSE_BRAC {$$=$3;}
;
...
c_length_field :
   SERIAL   INTVAL {$$= new length_spec(E_SERIAL,$2);}
   PARALLEL INTVAL {$$= new length_spec(E_PARALLEL,$2);}
;
...
| OPEN_BRAC c_ground_f_exp CLOSE_BRAC {$$=$2;}
;

c_binary_ground_f_exp :
    PLUS c_ground_f_exp c_ground_f_exp   { $$= new plus_expression($2,$3); }
|   HYPHEN c_ground_f_exp c_ground_f_exp { $$= new minus_expression($2,$3); }
|   MUL c_ground_f_exp c_ground_f_exp    { $$= new mul_expression($2,$3); }
